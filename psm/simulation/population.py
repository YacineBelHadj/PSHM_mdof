...
class Population:
    ...
    def generate_population(self):
        ...
        """
        pop_param =  settings.population.type1.param
        pop_set = settings.population.type1.info


        # Generate N sets of system parameters with random noise
        for i in range(pop_set['N']):
            system_variables = {}
            system_variables['mass'] = np.random.normal(pop_param['m_mean'], pop_param['m_std'])
            system_variables['stiffness'] = np.random.normal(pop_param['k_mean'], pop_param['k_std'])
            system_variables['stiffness'][0]=1e-6 + 0.001*np.random.normal(0,1e-6)
            system_variables['damping'] = np.random.normal(pop_param['c_mean'], pop_param['c_std'])
